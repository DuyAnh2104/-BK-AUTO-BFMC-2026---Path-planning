import networkx as nx
import matplotlib.pyplot as plt
import matplotlib.image as mpimg
from matplotlib.widgets import Slider, Button
import os

# --- CẤU HÌNH FILE ---
GRAPH_FILE = "Competition_track_graph.graphml"
IMG_FILE = "Competition_track_graph.png"

# --- KIỂM TRA FILE ---
if not os.path.exists(GRAPH_FILE) or not os.path.exists(IMG_FILE):
    print(f"LỖI: Không tìm thấy file {GRAPH_FILE} hoặc {IMG_FILE}")
    exit()

# --- LOAD DỮ LIỆU ---
G = nx.read_graphml(GRAPH_FILE)
pos = {}
all_x, all_y = [], []
for n, data in G.nodes(data=True):
    x = float(data.get('x', data.get('d0', 0)))
    y = float(data.get('y', data.get('d1', 0)))
    pos[n] = (x, y)
    all_x.append(x)
    all_y.append(y)

# Tính tâm của đồ thị (để zoom từ tâm ra cho chuẩn)
center_x = (min(all_x) + max(all_x)) / 2
center_y = (min(all_y) + max(all_y)) / 2
base_width = max(all_x) - min(all_x)
base_height = max(all_y) - min(all_y)

img = mpimg.imread(IMG_FILE)

# --- THIẾT LẬP GIAO DIỆN ---
fig, ax = plt.subplots(figsize=(14, 8))
plt.subplots_adjust(left=0.1, bottom=0.35) # Chừa chỗ trống bên dưới cho thanh trượt

# Vẽ Graph cố định (Mốc chuẩn)
nx.draw_networkx_nodes(G, pos, ax=ax, node_size=10, node_color='red')
# nx.draw_networkx_edges(G, pos, ax=ax, edge_color='cyan', alpha=0.3) 

# Vẽ ảnh nền ban đầu
initial_scale = 1.45 # Bắt đầu thử từ số to hơn lúc nãy
initial_dx = 0
initial_dy = 0

def calculate_extent(scale, dx, dy):
    # Công thức: Kích thước mới = Kích thước gốc * Scale
    # Dịch chuyển tâm đi dx, dy
    new_w = base_width * scale
    new_h = base_height * scale
    
    cx = center_x + dx
    cy = center_y + dy
    
    return [cx - new_w/2, cx + new_w/2, cy - new_h/2, cy + new_h/2]

img_obj = ax.imshow(img, extent=calculate_extent(initial_scale, 0, 0), aspect='auto', alpha=0.7)
ax.set_title("KÉO THANH TRƯỢT ĐỂ CHỈNH - SAU ĐÓ COPY SỐ Ở DƯỚI")

# --- TẠO CÁC THANH TRƯỢT (SLIDERS) ---
# Màu mè tí cho dễ nhìn
axcolor = 'lightgoldenrodyellow'

# Thanh chỉnh độ to nhỏ (Scale)
ax_scale = plt.axes([0.2, 0.2, 0.6, 0.03], facecolor=axcolor)
s_scale = Slider(ax_scale, 'Độ Phóng (Scale)', 1.0, 2.5, valinit=initial_scale, valstep=0.01)

# Thanh chỉnh trái phải (Dịch X)
ax_dx = plt.axes([0.2, 0.15, 0.6, 0.03], facecolor=axcolor)
s_dx = Slider(ax_dx, 'Dịch Ngang (X)', -10.0, 10.0, valinit=0, valstep=0.1)

# Thanh chỉnh lên xuống (Dịch Y)
ax_dy = plt.axes([0.2, 0.1, 0.6, 0.03], facecolor=axcolor)
s_dy = Slider(ax_dy, 'Dịch Dọc (Y)', -10.0, 10.0, valinit=0, valstep=0.1)

# --- HÀM CẬP NHẬT KHI KÉO ---
def update(val):
    scale = s_scale.val
    dx = s_dx.val
    dy = s_dy.val
    
    # Tính extent mới
    new_extent = calculate_extent(scale, dx, dy)
    
    # Cập nhật ảnh
    img_obj.set_extent(new_extent)
    fig.canvas.draw_idle()

# Gắn hàm update vào slider
s_scale.on_changed(update)
s_dx.on_changed(update)
s_dy.on_changed(update)

# --- NÚT IN KẾT QUẢ ---
resetax = plt.axes([0.8, 0.025, 0.1, 0.04])
button = Button(resetax, 'In Tọa Độ', color=axcolor, hovercolor='0.975')

def print_result(event):
    ext = calculate_extent(s_scale.val, s_dx.val, s_dy.val)
    print("\n" + "="*40)
    print("CHÚC MỪNG! ĐÂY LÀ SỐ CỦA ÔNG:")
    print(f"MY_EXTENT = [{ext[0]:.4f}, {ext[1]:.4f}, {ext[2]:.4f}, {ext[3]:.4f}]")
    print("="*40 + "\n")

button.on_clicked(print_result)

plt.show()